...
export function toggleChatView(m, id, key, peer) { // old r()
    ...
    return fetchDataAndRender(s + "chats.json", m, id, key, peer)
}

function fetchDataAndRender(e, m, id, key, peer) { //old _(e, m)
    var a, s;
    (async () => {
        let table, l
        if ("users" == m) {
    ...
    async function render(e, t) { //old a = function
        ...
        if (false) {
            ...
            n = 0
            l.forEach(async function (t, e) {
                await insertMessage(n,t,f,v,m)
            })
        }
        ...
        $('#chatinput-form').unbind('submit').on('submit', function (e) {
            e.preventDefault()
            return sendChatMessage(e, m, id, peer)
...
export function isChatOpen() {
    return document.querySelector("#users-chat").style.display == "block"
}

//get chat id
export function getChatId() {
    return document.querySelector("#users-chat").getAttribute("data-id")
}

// insert message in chat
export function insertMessage(n,t,f,v,m) {
    //console.log(n,t,f,v,m)
    var a, s, i;
    if (0 < n) {
        --n
    } else {
        a = t.from_id == 1 ? " right" : " left"
        s = f.find(function (e) {
            return e.id == t.from_id
        })
        //console.log(s)
        i = '<li class="chat-list' + a + '" id=' + t.id + '><div class="conversation-list">',
        v != t.from_id && (i += '<div class="chat-avatar"><img src="' + (s?.profile || IMG_DUMMY) + '" onerror="this.src = this.src" alt=""></div>'),
